...
{
  CalCtPtRatio();
  EepromRead(SCROLL_LOCK_LOC,2,EXT_EEPROM,(uint8_t *)&DisplaySetup.DisplayScrollStatus );
  EepromRead(PASSWORD_SAV_LOC,2,EXT_EEPROM,(uint8_t *)&MeterPassword );
...
void DisplayParameterUpdate(void)
{
  ...
  uint8_t i;
  for(i=0;i<100;i++)DisplayParameterBuffer[i]= SCREEN_SEQ_3P_MAIN[i];
